...
Usage:
    python3 autorecon.py -t <IP>
    python3 autorecon.py -t <IP> --all
    python3 autorecon.py -t <IP> --scans port vuln os banner
    python3 autorecon.py -t <IP> --output results/
"""
...
def log(msg, level="INFO"):
    ...
    prefix = {
        ...
        "ERR":     f"{C.RED}[-]{C.RESET}",
        "SECTION": f"{C.BLUE}{C.BOLD}[>]{C.RESET}",
    }.get(level, "[?]")
...
def print_scan_menu():
    print(f"\n{C.BOLD}Available Scan Modules:{C.RESET}")
...
def print_summary(results: list, output_dir: Path, save: bool):
...
def main():
    ...
    parser.add_argument("--scans",          nargs="+", metavar="SCAN",
                        help=f"Scan modules to run. Choices: {', '.join(ALL_SCAN_KEYS)}")
    ...
    if not check_nmap():
        log("nmap not found! Install it: sudo apt install nmap", "ERR")
        sys.exit(1)
    ...
    print_summary(results, output_dir, save)
